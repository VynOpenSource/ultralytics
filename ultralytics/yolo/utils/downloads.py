...
def safe_download(url,
    ...
    """
    f = os.path.join(dir, url2file(url)) if dir else Path(file)  # URL converted to filename
    if '://' not in str(url) and Path(url).is_file():  # URL exists ('://' check required in Windows Python<3.10)
        f = Path(url)  # filename
    elif not f.is_file():  # URL and file do not exist
        assert dir or file, 'dir or file required for download'
        #f = dir / url2file(url) if dir else Path(file)
        f = os.path.join(dir, url2file(url)) if dir else Path(file)
...
def attempt_download_asset(file, repo='ultralytics/assets', release='v0.0.0', dir=None):
    ...
    if file.exists():
        ...
        if str(file).startswith(('http:/', 'https:/')):  # download
            ...
            if Path(file).is_file():
                ...
                safe_download(url=url, file=file, min_bytes=1E5, dir=dir)
            return file
        ...
        if name in assets:
            safe_download(url=f'https://github.com/{repo}/releases/download/{tag}/{name}', file=file, min_bytes=1E5, dir=dir)
