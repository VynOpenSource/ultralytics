...
def get_statistic_of_input_data(labels, class_rate, shuffle_class, mapping_classes, data_names):
    """
    Isaac's Addition
    ...
    for i, value in enumerate(labels):
        ...
        if len(cls) == 0:
            ...
            cls = np.unique(cls)
        for label in cls:
            label_indices.setdefault(int(label), []).append(i)

    if not class_rate and shuffle_class and mapping_classes:
        nc = len(label_indices)
        class_rate = {i: float(i + 1) / nc for i in range(nc)}

    if class_rate:
        data_names[-1] = 'negative'
        inverse_name = {name: index for index, name in data_names.items()}
        class_rate = {inverse_name[key]: value for key, value in class_rate.items() if
                           inverse_name.get(key) in label_indices}
        # if self.mapping_classes:
        #    self.inverse_mapping = {value: key for key, value in self.mapping_classes.items()}
        prob_aux = class_rate.copy()

        class_keys = list(prob_aux.keys())
        values = [prob_aux[key] for key in class_keys]
        values = np.cumsum(values) / np.sum(values)
        class_rate = {key: values[i] for i, key in enumerate(class_keys)}

    return label_indices, class_rate, data_names
...
def check_det_dataset(dataset, autodownload=True):
    ...
    # Isaac's Addition
    if 'nc_complete' not in data:
        data['nc_complete'] = data['nc']
