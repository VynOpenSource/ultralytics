...
# Isaac's Addition
from ultralytics.yolo.utils import initialise_dict, remove_ambiguous_classes, get_indices
...
def create_dataloader(path,
    ...
    with torch_distributed_zero_first(rank):  # init dataset *.cache only once if DDP
        dataset = LoadImagesAndLabels(
            ...
            min_items=min_items,
            prefix=prefix,
            ambiguous_classes=ambiguous_classes, # Isaac's Addition all the parameters down
...
def img2label_paths_original(img_paths):
...
def img2label_paths(img_paths):
    ...
    for x in img_paths:
        ...
        l = os.path.join(root, 'labels', dirname, basename)
        labels.append(l.rsplit('.', 1)[0] + '.txt')
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self,
        ...
        # Isaac's Addition all the parameters down
    ...
    def get_statistic_of_input_data(self):
        ...
        if self.class_rate:
            self.class_rate = {key: value for key, value in self.class_rate.items() if key in self.label_indices}
