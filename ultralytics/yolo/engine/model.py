...
class YOLO:
    ...
    def _load(self, weights: str, task=None):
        ...
        Args:
            ...
            task (str | None): model task
        """
        root_to_save_model = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'assets')
        suffix = Path(weights).suffix
        if suffix == '.pt':
            self.model, self.ckpt = attempt_load_one_weight(weights, dir=root_to_save_model)
    ...
    def train(self, **kwargs):
        ...
        #   overrides = kwargs['cfg'] if isinstance(kwargs['cfg'], dict) else yaml_load(check_yaml(kwargs['cfg']))
        overrides.update(kwargs)
