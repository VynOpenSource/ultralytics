...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, pred_bboxes, pred_scores, gt_bboxes, gt_cls, gt_groups, masks=None, gt_mask=None):
        ...
        # ISAAC's ADDITION
        # The issue is that some of the values are NaN and that gives an error. The solution is to change the
        # values to be 0, so the confidence
        def nan2zero(p, indexes=None):
            """
            Convert complete vectors in the last dimension to 0 if a nan is found
            """
            if p is not None:
                if indexes is None:
                    indexes = torch.isnan(torch.sum(p, dim=-1))
                dims = [1] * (len(p.shape) - 1) + [p.shape[-1]]
                indexes = indexes[..., None].repeat(*dims)
                return torch.where(indexes, torch.zeros_like(p), p)
            return p

        #indexes = torch.isnan(torch.sum(pred_scores, dim=-1)) + torch.isnan(torch.sum(pred_bboxes, dim=-1))
        #pred_scores = nan2zero(pred_scores, indexes)
        #pred_bboxes = nan2zero(pred_bboxes, indexes)
        # ----------------------------------------------------------------------
